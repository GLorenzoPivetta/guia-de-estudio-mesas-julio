...
- **¿Qué es un ciclo de vida?**
	- *Un ciclo de vida describe la secuencia de pasos de un proyecto desde que se plantea la problemática hasta que se realiza la entrega del mismo.*
- **¿Qué diferencia hay entre lineal, iterativo e incremental?**
	- *Un ciclo lineal tiene etapas definidas secuencialmente, no se genera un entregable técnico hasta el fin del ciclo. Un ciclo iterativo repite fases antes de avanzar para corregir y adaptar scope. Y un ciclo incremental, además de repetir fases, con cada nueva fase, añade funcionalidades al sistema con cada iteración.*
- **¿Por qué el sistema completo aparece tarde en cascada?**
	- *Porque en cascada no se suele mostrar el sistema hasta su entrega, si no que se van mostrando gráficos o reportes que informan los avances.*
- **¿Qué ventaja tiene un prototipo ejecutable?**
	- *Permite que los clientes prueben un flujo o interacción y den feedback rápido y conciso del proyecto.*
- **¿Por qué un ciclo iterativo no significa falta de planificación?**
	- *Que un ciclo sea iterativo no significa que no esté planificado, si no que los requerimientos no son claros y existe la posibilidad de que se tengan que realizar cambios sobre la marcha para ajustar así el prototipo al resultado final esperado.
